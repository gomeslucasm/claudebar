...
export async function profile(args) {
    ...
    // ── current ──
    if (sub === 'current') {
        console.log(resolveProfileName(config));
        return;
    }
    // ── use ──
    if (sub === 'use') {
        ...
        if (!config.profiles[name]) {
            console.log(m.pm.unknown(name, names().join(', ')));
            process.exit(1);
        }
    ...
    if (sub === 'remove' || sub === 'rm' || sub === 'delete') {
        ...
        if (names().length <= 1) {
            console.log(m.pm.cannotRemoveLast);
            process.exit(1);
        }
        ...
        if (config.override?.profile === name)
            delete config.override;
        if (config.activeProfile === name)
            config.activeProfile = names()[0];
        saveConfig(config);
        p.log.success(m.pm.removed(name));
        if (pruned)
            p.log.message(m.pm.prunedSwitches(pruned));
        return;
    }
    // ── switch add / remove ──
    if (sub === 'switch') {
        ...
        if (action === 'add') {
            ...
            if (!isValidTime(rawTime)) {
                console.log(m.pm.badTime);
                process.exit(1);
            }
            if (!config.profiles[prof]) {
                console.log(m.pm.unknown(prof, names().join(', ')));
                process.exit(1);
            }
